...
class CustomerAddressController {
    ...
    public update = async (req: Request, res: Response, next: NextFunction) => {
        const model: Create = req.body as Create;
        model.customer_id = req.body.customer_id || req.ref_id as number;
        model.seller_id = req.seller_id as number;
        model.address_id = req.params.addressId as any;
        try {
    ...
    //     const seller_id = req.seller_id as number;
    //     try {
    //         const result = await this.customerAddressServices.update(addressId, customerId, seller_id);
    //         if (result instanceof Error && result.field)
    //             return sendResponse(res, result.status, result.message, null, result.field);
    //         if (result instanceof Error)
    //             return sendResponse(res, result.status, result.message);
